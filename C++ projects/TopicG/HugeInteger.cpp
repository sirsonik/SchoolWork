...
Only modificaiton is now the overloaded subscript operator will throw an out_of_range
exception when an out of range subscript is called.
*/
...
*/
int HugeInteger::operator[](const int slot) const
{
	if (slot > Size)
		throw out_of_range( "Subscript for HugeInteger object is invalid" );
